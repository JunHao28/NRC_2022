class Tasks:
    ...
    def collectChemical(self, direction, degree, special=0):
        ...
        if special == 0:
            ...
            self.movement.gyrodegree(-60, -180, decel=False, override=degree+90)
            self.wait(0.2)
            ...
            self.movement.gyrodegree(150, 100, maximumSpeed=300, accelDist=1, )
            self.wait(0.3)
            self.movement.turn(0, 90 if direction == 2 else -90)
            self.wait(0.3)
            self.movement.gyrodegree(-60, -100, decel=False)
            self.wait(0.3)
            self.motord.run_target(1500, -250)
            self.basic.stop()
            self.movement.gyrodegree(60, 100, decel=False)
