class Tasks:
    ...
    def __init__(self, ev3, motor, sensor, basic, human):
        self.ev3 = ev3
        self.motorb = motor[0]
        self.motorc = motor[1]
        self.motord = motor[2]
    ...
    def collectChemical(self, direction, special, corner):
        ...
        if special == False:
            if corner == False:
                self.gyrodegree(40, 200)
                self.turn(0, (-2*direction+3)*90, oneWheel=direction)
                self.gyrodegree(-160, 100)
                self.motord.run_target(-1500, 200)
                self.stop()
                time.sleep(0.5)
                self.gyrodegree(160, 200)
                self.turn(0, (-2*direction+3)*-90, oneWheel=direction)
            elif corner == True:
                # Change
                self.gyrodegree(40, 200)
