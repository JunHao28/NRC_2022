...
def section5(robot):
    ...
    robot.pause(0.5)
    robot.movement.gyroTillSense(-100, lambda: robot.tasks.checkColour(robot.basic.sense(2), robot.basic.sense(4), 2, 2, special=2), stopAfter=-100, override=0, stop=True)
    
    robot.movement.turn(0, - robot.basic.sense(0), oneWheel=1)
    robot.movement.gyrodegree(-300, -190, decel=False, stop=False, override=0)
    robot.basic.beep()
    robot.movement.lineTrackingTillSense(robot.colour["line_tracking"], 200, lambda: robot.tasks.checkColour(robot.basic.sense(3), robot.basic.sense(5), 2, 2), stopAfter=200)
